...
export default function ReminderDetails() {
  ...
  return (
    <ThemedContainer>
      ...
      <Text>
        Current Interval:{" "}
        {intervalStart ? intervalStart.toLocaleString() : "N/A"} to{" "}
        {intervalEnd ? intervalEnd.toLocaleString() : "N/A"} for {intervalIndex}
      </Text>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Card variant="outline" style={{ marginBottom: 10, padding: 10 }}>
              <Text>
                Scheduled At: {new Date(item.scheduled_at).toLocaleString()}
              </Text>
              <Text>Status: {item.status || "pending"}</Text>
              {item.response_at && (
                <Text>
                  Responded At: {new Date(item.response_at).toLocaleString()}
                </Text>
              )}
            </Card>
          )}
